        ...
        <nav class="navbar navbar-default" style="margin-bottom: 0px">
            <div class="container-fluid">
                <div class="navbar-header">
                    <a class="navbar-brand" href="#"><i class="fa fa-info-circle" aria-hidden="true"></i> Daftar Informasi Lowongan Kerja</a>
                </div>
                    ...
                    <?php
                        ...
                        foreach ($rows as $key => $value) {
                            echo "
                                <tr>
                                    <td>{$no}</td>
                                    <td>{$value->judul_karir}</td>
                                    <td>{$value->nama_spes}</td>
                                    <td>{$value->perusahaan_karir}</td>
                                    <td>{$value->deadline_mod}</td>
                                    <td>
                                        <a href='?module={$this->module}&act=edit&id={$value->id_karir}'>
                                            <input type='image' src='images/icn_edit.png' title='Edit'>
                                        </a> &nbsp;&nbsp;&nbsp;&nbsp;
                                        <a href='?module={$this->module}&act=delete&id={$value->id_karir}' onclick=\"return confirm('Apakah anda yakin menghapus data ini?')\">
