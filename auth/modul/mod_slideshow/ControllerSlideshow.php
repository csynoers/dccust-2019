<?php
    class ControllerSlideshow
    {
        ...
        public function option_fakultas($id=NULL)
        {
            if ( empty($id) ) {
                $fakultas= [];
                foreach ($this->Model->get_fakultas() as $key => $value) {
                    $fakultas[] = "<option value='{$value->id_fakultas}'>{$value->fakultas}</option>"; 
                }
                return $fakultas= implode('',$fakultas);
            } else {
                $fakultas= [];
                foreach ($this->Model->get_fakultas() as $key => $value) {
                    $fakultas[] = "<option value='{$value->id_fakultas}' ".($value->id_fakultas==$id? 'selected' : NULL ).">{$value->fakultas}</option>"; 
                }
                return $fakultas= implode('',$fakultas);
            }
        }
