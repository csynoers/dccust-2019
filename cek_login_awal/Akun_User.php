		...
		public static function cekStatusLogin(){
			...
			$cekLogin=FALSE;
				if(isset($_SESSION['login_status']) && $_SESSION['login_status'] == TRUE){					
					...
					$st->bindValue(":id_session", $_SESSION['id_session'] , PDO::PARAM_STR);
					$st->execute();
					$hasil = $st->fetch();
					$koneksi = null;
					$cekLogin = isset($hasil['id_alumni']) && !is_null($hasil['id_alumni']) && !empty($hasil['id_alumni']) ? TRUE : FALSE ;					
				}
				else{
					$cekLogin = FALSE;					
				}
		...
		public function suntingAkun(){
	 	...
	 	public function login(){
			...
			if(crypt($this->password, $cekDb['password']) != $cekDb['password']){
				...
				$_SESSION['id_session'] = $this->id_session;

				//mengisi session				
				$_SESSION['username'] = $this->username;
				$_SESSION['login_status'] = TRUE;
				$_SESSION['id_alumni'] = $cekDb['id_alumni'];
